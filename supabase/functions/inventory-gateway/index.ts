...
async function change(operation: string, payload: Row, user: AppUser | null) {
  ...
  if (operation === "create_erp_project" || operation === "update_erp_project") { requireRole(user,["admin","operator"]); const isUpdate=operation==="update_erp_project",id=isUpdate?uuid(payload.id):null,rowVersion=isUpdate?Number(payload.row_version):null,name=limited(payload.name,120),customer_id=uuid(payload.customer_id),project_type=text(payload.project_type),status=limited(payload.status,40),description=nullable(payload.description,2000),note=nullable(payload.note,1000),estimated_cost=payload.estimated_cost === "" ? null : nonNegative(payload.estimated_cost),worker_user_ids=Array.isArray(payload.worker_user_ids)?payload.worker_user_ids.map(uuid):[]; if((isUpdate&&(!id||!Number.isInteger(rowVersion)||Number(rowVersion)<1))||!name||!customer_id||!["construction","maintenance"].includes(project_type)||!status||description===null||note===null||estimated_cost===undefined||worker_user_ids.some(workerId=>!workerId)||new Set(worker_user_ids).size!==worker_user_ids.length||worker_user_ids.length>30) throw new Error("請完整填寫專案資料與有效的負責人。"); return rpc("upsert_erp_project_with_workers_v1",{p_id:id,p_row_version:rowVersion,p_name:name,p_customer_id:customer_id,p_project_type:project_type,p_status:status,p_description:description||null,p_estimated_cost:estimated_cost,p_note:note||null,p_worker_user_ids:worker_user_ids,p_actor:actor}); }
