...
const datasets: Record<string, DatasetDefinition> = {
 ...
 access_projects:{path:"projects?select=id,name,project_code,customer_id&order=project_code.asc,id.asc",adminOnly:true,paged:true},
  projects: { path: "projects?select=id,name,project_code,customer_id,project_type,status,project_date,assigned_to,description,estimated_cost,actual_cost,started_on,completed_on,note,created_at,updated_at,row_version,source,updated_by&order=updated_at.desc,id.asc", paged: true },
  ...
  receipts: { path: "stock_receipts?select=stock_receipt_customers(customer_id),id,receipt_date,inventory_item_id,quantity,supplier_id,supplier,note,row_version,created_at,updated_at,source,updated_by&order=receipt_date.desc,created_at.desc,id.desc", paged: true },
...
const queryDefinitions: Record<string, { table: string; select: string; search: string[]; sort: Record<string,string> }> = {
  ...
  projects: { table: "projects", select: "id,name,project_code,customer_id,project_type,status,project_date,assigned_to,description,estimated_cost,note,created_at,updated_at,row_version", search: ["project_code","name","assigned_to","status"], sort: { code:"project_code", name:"name", status:"status", date:"created_at" } },
  ...
  receipts: { table: "stock_receipts", select: "stock_receipt_customers(customer_id),id,receipt_date,inventory_item_id,quantity,supplier_id,supplier,note,row_version,created_at,updated_at", search: ["receipt_date","supplier","note"], sort: { date:"receipt_date", supplier:"supplier", created:"created_at" } },
...
async function change(operation: string, payload: Row, user: AppUser | null) {
  ...
  if (operation === "upsert_customer_project_work_log") {
    ...
    if((text(payload.id)&&!id)||(id&&(!Number.isInteger(rowVersion)||Number(rowVersion)<1))||(text(payload.project_id)&&!project_id)||!customer_id||!project_name||!log_date||!["工程施工","維修紀錄","維護保養","送貨","文書作業"].includes(work_type)||summary===null||worker_user_ids.some(workerId=>!workerId)||new Set(worker_user_ids).size!==worker_user_ids.length||worker_user_ids.length>30) throw new Error("請完整填寫工作日誌、工作類型與有效的施工人員。");
  ...
  if (operation === "upsert_project_site_entry") {
    ...
    if(module==="logs"){
      ...
      if(!log_date||!title||summary===null||!["工程施工","維修紀錄","維護保養","送貨","文書作業"].includes(work_type)||worker_user_ids.some(workerId=>!workerId)||new Set(worker_user_ids).size!==worker_user_ids.length||worker_user_ids.length>30)throw new Error("請完整填寫工作日誌、工作類型與有效的施工人員。");
