...
async function change(operation: string, payload: Row, user: AppUser | null) {
  ...
  if (operation === "upsert_customer_project_work_log") {
    ...
    if((text(payload.id)&&!id)||(id&&(!Number.isInteger(rowVersion)||Number(rowVersion)<1))||(text(payload.project_id)&&!project_id)||!customer_id||!project_name||!log_date||!["工程施工","維修紀錄","維護保養","送貨"].includes(work_type)||summary===null||worker_user_ids.some(workerId=>!workerId)||new Set(worker_user_ids).size!==worker_user_ids.length||worker_user_ids.length>30) throw new Error("請完整填寫工作日誌、工作類型與有效的施工人員。");
  ...
  if (operation === "upsert_project_site_entry") {
    ...
    if(module==="logs"){
      ...
      if(!log_date||!title||summary===null||!["工程施工","維修紀錄","維護保養","送貨"].includes(work_type)||worker_user_ids.some(workerId=>!workerId)||new Set(worker_user_ids).size!==worker_user_ids.length||worker_user_ids.length>30)throw new Error("請完整填寫工作日誌、工作類型與有效的施工人員。");
