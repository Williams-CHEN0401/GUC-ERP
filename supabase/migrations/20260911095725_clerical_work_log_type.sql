begin;
-- Extend the existing shared type mapping; keep project/log synchronization and RPC privileges.
-- No data rewrite, master tables, Auth or RBAC changes. Apply only when production rollout is authorized.
alter table public.projects drop constraint projects_project_type_check;
alter table public.projects add constraint projects_project_type_check check (project_type in ('construction','repair','maintenance','delivery','clerical'));
alter table public.site_work_logs drop constraint site_work_logs_work_type_check;
alter table public.site_work_logs add constraint site_work_logs_work_type_check check (work_type in ('工程施工','維修紀錄','維護保養','送貨','文書作業'));
...
begin
  ...
  v_project_type := case new.work_type
    ...
    when '送貨' then 'delivery' when '文書作業' then 'clerical'
  ...
  if p_id is not null then
    ...
    if not found then
      ...
      from public.create_project_auto_number_v1(
        ...
        case p_work_type when '工程施工' then 'construction' when '維修紀錄' then 'repair' when '送貨' then 'delivery' when '文書作業' then 'clerical' else 'maintenance' end,
...
begin
  ...
  if p_project_type not in ('construction', 'repair', 'maintenance', 'delivery', 'clerical') then
...
begin
  ...
  if p_work_type is null or p_work_type not in ('工程施工', '維修紀錄', '維護保養', '送貨', '文書作業') then
...
begin
  ...
  if p_project_type not in ('construction','repair','maintenance','delivery','clerical') then raise exception '專案類型不正確。'; end if;
