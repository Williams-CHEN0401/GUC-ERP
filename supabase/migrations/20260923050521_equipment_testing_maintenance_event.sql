-- Equipment testing belongs to maintenance events, not project/work-log types.
-- Existing rows, repair-item rules, function signature and privileges are unchanged.
begin;
...
alter table public.maintenance_events add constraint maintenance_events_type_check
  check (event_type in ('SOFTWARE_CONFIG','LINE_REPAIR','LINE_REPLACEMENT','REPAIR','REPLACEMENT','EQUIPMENT_TEST',
...
declare
  ...
  v_before_repair public.repair_items;
...
begin
  ...
  loop
    ...
    if v_event_type not in (
      'SOFTWARE_CONFIG','LINE_REPAIR','LINE_REPLACEMENT','REPAIR','REPLACEMENT','EQUIPMENT_TEST',
      'INSTALLATION','MAINTENANCE','PROGRAM_CONFIG','INSPECTION','OTHER'
    )
    ...
    if v_event_type not in ('SOFTWARE_CONFIG','LINE_REPAIR','LINE_REPLACEMENT','REPAIR','REPLACEMENT','EQUIPMENT_TEST')
      and (v_event_id is null or v_event_type is distinct from v_before.event_type) then
      raise exception '請選擇有效事件類型；舊分類僅可保留原紀錄。';
    end if;
    ...
    if v_register_repair then
      insert into public.repair_items(
        customer_id,department_id,inventory_item_id,notes,received_on,quantity,status,issue_description,
        source_maintenance_event_id,source,updated_by
      ) values(
        p_customer_id,(select p.department_id from public.projects p join public.site_work_logs l on l.project_id=p.id where l.id=v_work_log_id),v_inventory_item_id,v_notes,p_log_date,null,null,v_cause,
  ...
  -- Keep linked repairs on the parent work-log date, including a date-only edit.
