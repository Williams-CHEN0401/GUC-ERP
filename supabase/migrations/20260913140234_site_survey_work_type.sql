...
-- Add site_survey / 場勘 to the existing shared type mapping.
-- These eight CREATE OR REPLACE definitions extend the 20260911095725 definitions;
-- existing function signatures, security attributes and EXECUTE grants are preserved.
...
alter table public.projects add constraint projects_project_type_check check (project_type in ('construction','repair','maintenance','delivery','clerical','site_survey'));
alter table public.site_work_logs drop constraint site_work_logs_work_type_check;
alter table public.site_work_logs add constraint site_work_logs_work_type_check check (work_type in ('工程施工','維修紀錄','維護保養','送貨','文書作業','場勘'));
...
begin
  ...
  v_project_type := case new.work_type
    ...
    when '送貨' then 'delivery' when '文書作業' then 'clerical' when '場勘' then 'site_survey'
...
begin
  ...
  if p_work_type is null or p_work_type not in ('工程施工', '維修紀錄', '維護保養', '送貨', '文書作業', '場勘') then
...
begin
  ...
  if p_project_type not in ('construction','repair','maintenance','delivery','clerical','site_survey') then raise exception '專案類型不正確。'; end if;
