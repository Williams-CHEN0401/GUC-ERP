-- Preserve service_role table restrictions and existing RPC grants/security modes.
-- Only the already-authorized maintenance implementation inserts new repairs.
begin;
  ...
  loop
    ...
    if v_register_repair then
      insert into public.repair_items(
        ...
        p_customer_id,(select p.department_id from public.projects p join public.site_work_logs l on l.project_id=p.id where l.id=v_work_log_id),v_inventory_item_id,v_notes,null,null,null,v_cause,
