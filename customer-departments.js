...
function customerSelectorFields(customerId="",category="",departmentId="",{legacy=false,required=true}={}){
 return `<div class="customer-department-selector span-2" data-customer-selector data-legacy-customer="${legacy?esc(customerId):""}" data-legacy-department="${legacy?esc(departmentId):""}" data-required="${required}">${customerCategoryOptions(category)}${customerOptions(customerId,category)}<label>科室<select name="departmentId">${departmentOptionsMarkup(customerId,departmentId,{legacy})}</select></label><label class="customer-selector-search">搜尋客戶<input type="search" name="customerSelectorSearch" placeholder="客戶編號或名稱" autocomplete="off"></label><input type="hidden" name="categoryId" value="${esc(state.customerCategories.find(row=>row.code===category)?.id||"")}"></div>`;
}
