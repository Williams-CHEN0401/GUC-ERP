...
function context(){
 ...
 const ctx=vm.createContext({state:{siteData:{logs:[saved]}},savedWorkLogId:r=>r.work_log.id,byId:(rows,id)=>rows.find(row=>row.id===id),maintenanceSaveMatches:()=>true,PREVIEW_MODE:true});
 vm.runInContext(named('workLogMatchesSave')+'\n'+named('repairVisitName')+'\n'+app.slice(app.indexOf('async function verifySavedWorkLog('),app.indexOf('function nasFormRequest(')),ctx);
