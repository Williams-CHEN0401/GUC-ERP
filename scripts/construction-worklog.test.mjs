...
const functions=['isMaintenanceWorkLog','workLogSelectableProjects','workLogProjectTitleField','syncWorkLogProjectNames'];
...
test('new work-log initial and refreshed choices exclude completed while preserving customer and grants',()=>{
 ...
 const list={tagName:'DATALIST'},form={elements:{customerId:{value:'c1'}},querySelector:selector=>selector==='#workLogProjectNames'?list:null};
 const ctx=vm.createContext({state,esc:v=>v||'',workLogOriginalProject:()=>null,workLogFormCustomerId:()=>form.elements.customerId?.value||'',projectMatchesDepartment:()=>true,workLogTitleDepartment:()=>undefined,modalDepartmentFilter:()=>undefined,document:{querySelector:()=>form}});
