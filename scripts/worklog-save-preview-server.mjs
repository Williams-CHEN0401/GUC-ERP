...
import {formSyncDatabase,syncOperatorId} from './form-sync-fixture.mjs';
const root=fileURLToPath(new URL('../',import.meta.url));
export async function createWorklogTestServer({workflow=false,titlePicker=false,contractCatalog=false,assignments=false,dailyTypes=false,independent=false,formSync=false}={}){
 ...
 const context=vm.createContext({AsyncLocalStorage,performance,URL,URLSearchParams,Request,Response,Headers,DOMException,AbortController,setTimeout,clearTimeout,crypto,console,Deno:{env:{get:()=>''},serve:fn=>handler=fn}});
 vm.runInContext(source,context);
 const operator={...currentUser,id:syncOperatorId,username:'fixture-operator',display_name:'隔離測試員 B',role:'operator'};
 const requestUser=request=>formSync&&request.headers.get('authorization')?.includes('isolated-operator')?operator:currentUser;
 ...
 const all=async table=>(await db.query('select * from '+table)).rows.map(row=>Object.fromEntries(Object.entries(row).map(([key,value])=>[key,value instanceof Date&&['log_date','occurred_at','received_on','project_date','receipt_date','pickup_date'].includes(key)?value.toISOString().slice(0,10):value])));
const snapshot=async (scope,user=currentUser)=>({scope,current_user:user,...(formSync?{work_content_types:(await db.query('select * from erp_work_content_types_v1()')).rows}:{}),customers:await all('customers'),customer_departments:await all('customer_departments'),customer_categories:[{id:'school',code:'school',name:'學校機關'}],projects:(await all('projects')).filter(p=>!independent||scope==='worklogs'||!p.deleted_at),sites:await all('sites'),site_workers:formSync?[currentUser,operator]:[currentUser],site_work_logs:await all('site_work_logs'),site_work_log_workers:await all('site_work_log_workers'),maintenance_events:await all('maintenance_events'),maintenance_event_equipment:await all('maintenance_event_equipment'),maintenance_event_workers:await all('maintenance_event_workers'),repair_items:await all('repair_items'),equipment_registry:[],contract_service_types:contractCatalog?await all('contract_service_types'):[{id:ids.service,code:'computer',name:'電腦設備（測試承攬）',is_active:true,sort_order:1}],customer_contract_services:await all('customer_contract_services'),categories:formSync?await all('product_categories'):[{id:ids.category,name:'電腦設備',is_active:true}],items:formSync?await all('inventory_items'):[{id:ids.item,category_id:ids.category,item_name:'查修測試電腦',inventory_code:'TEST-001',unit:'台'}],pickups:assignments||independent?await all('pickup_records'):[],errors:[]});
 const server=createServer(async(req,res)=>{
  const url=new URL(req.url,'http://127.0.0.1');res.setHeader('Cache-Control','no-store');
  try{
   if(url.pathname==='/api/public-config'){res.setHeader('Content-Type','application/javascript');res.end('globalThis.GUC_PUBLIC_CONFIG={};sessionStorage.setItem("GUC_ERP_ACCESS_TOKEN",new URLSearchParams(location.search).get("test_user")==="B"?"isolated-operator":"isolated-fixture-only");');return;}
   if(url.pathname==='/api/inventory'){
    ...
    if(req.method==='GET'){const data=await snapshot(url.searchParams.get('scope')||'session',requestUser(new Request('http://127.0.0.1',{headers:req.headers}))),links=await all('stock_receipt_customers');data.suppliers=await all('suppliers');data.receipts=(await all('stock_receipts')).map(row=>({...row,stock_receipt_customers:links.filter(link=>link.stock_receipt_id===row.id)}));res.end(JSON.stringify(formSync&&url.searchParams.get('options_only')==='1'?context.referenceSnapshot(data):data));return;}
    let body='';for await(const chunk of req){body+=chunk;if(body.length>1000000)throw Error('Too large');}
    if(!['upsert_customer_project_work_log','create_erp_project','update_erp_project','upsert_repair_item','create_stock_receipt_batch','update_stock_receipt','create_customer_department','update_customer_department','deactivate_customer_department',...(formSync?['create_product_category','create_inventory_item_batch']:[]),...(independent?['update_pickup','create_pickup_batch','delete_erp_project']:[]),...(contractCatalog?['create_contract_service_type','update_contract_service_type','delete_contract_service_type','update_customer']:[]),...(assignments?['create_work_assignment','complete_work_assignment','acknowledge_work_assignment']:[])].includes(JSON.parse(body).operation)){res.writeHead(403);res.end('{"error":"隔離環境僅允許核准的測試操作"}');return;}
    const result=await handler(new Request('http://127.0.0.1/inventory-gateway',{method:'POST',headers:req.headers,body}));res.writeHead(result.status);res.end(await result.text());return;
   }
