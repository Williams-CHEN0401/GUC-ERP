...
export async function createWorklogTestServer({workflow=false,titlePicker=false,contractCatalog=false,assignments=false,dailyTypes=false,independent=false}={}){
 const db=await (independent?independentDatabase():dailyTypes?dailyTypeDatabase():assignments?assignmentDatabase():contractCatalog?contractCatalogDatabase():titlePicker?titlePickerDatabase():departmentDatabase()),calls=[];
...
const snapshot=async scope=>({scope,current_user:currentUser,customers:await all('customers'),customer_departments:await all('customer_departments'),customer_categories:[{id:'school',code:'school',name:'學校機關'}],projects:(await all('projects')).filter(p=>!independent||scope==='worklogs'||!p.deleted_at),sites:await all('sites'),site_workers:[currentUser],site_work_logs:await all('site_work_logs'),site_work_log_workers:await all('site_work_log_workers'),maintenance_events:await all('maintenance_events'),maintenance_event_equipment:await all('maintenance_event_equipment'),maintenance_event_workers:await all('maintenance_event_workers'),repair_items:await all('repair_items'),equipment_registry:[],contract_service_types:contractCatalog?await all('contract_service_types'):[{id:ids.service,code:'computer',name:'電腦設備（測試承攬）',is_active:true,sort_order:1}],customer_contract_services:await all('customer_contract_services'),categories:[{id:ids.category,name:'電腦設備',is_active:true}],items:[{id:ids.item,category_id:ids.category,item_name:'查修測試電腦',inventory_code:'TEST-001',unit:'台'}],pickups:assignments||independent?await all('pickup_records'):[],errors:[]});
 ...
 return{server,db,calls,snapshot,...(assignments||independent?{currentUser}: {})};
}
