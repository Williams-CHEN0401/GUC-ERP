-- Read-only snapshot of relevant production function definitions, 2026-09-15.
-- Used only in the isolated cross-system regression fixture; contains no business data.
...
CREATE OR REPLACE FUNCTION public.quotable_work_types_v1()
 RETURNS TABLE(code text, label text)
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO ''
AS $function$
  values ('construction','工程施工'), ('repair','維修／查修'), ('delivery','送貨');
$function$
;

CREATE OR REPLACE FUNCTION public.quotation_project_department_guard_v1()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
begin
 if new.department_id is distinct from old.department_id and exists(select 1 from public.quotations where project_id=old.id) then
