...
test("project and work-log types share labels while preserving canonical database values", () => {
  assert.match(app, /PROJECT_WORK_TYPES\s*=\s*\[\["construction", "工程施工", "工程施工"\], \["repair", "維修\/查修", "維修紀錄"\], \["maintenance", "維護保養", "維護保養"\], \["delivery", "送貨", "送貨"\], \["clerical", "文書作業", "文書作業"\]\]/);
  ...
  assert.match(gateway, /\["construction","repair","maintenance","delivery","clerical"\]/);
  assert.match(gateway, /\["工程施工","維修紀錄","維護保養","送貨","文書作業"\]/);
});
