...
test("phone Excel import is row-rollback safe, logged, service-role only, and prevents orphan field endpoints", () => {
  ...
  for (const marker of ["phone_type_match_status", "import_phone_field_rows_v1", "phone_type_matched", "failure_reasons", 'operation === "import_phone_terminal_rows"']) assert.ok(edge.includes(marker), `missing exact type matching/result marker: ${marker}`);
  ...
  const fieldMigration = readFileSync(new URL("../supabase/migrations/20260911092521_preserve_field_terminal_slots.sql", import.meta.url), "utf8");
  assert.match(fieldMigration, /security invoker/);
  assert.match(fieldMigration, /v_type<>v_system_type/);
  assert.match(fieldMigration, /已依同號碼系統端補入/);
  assert.match(fieldMigration, /from public,anon,authenticated/);
