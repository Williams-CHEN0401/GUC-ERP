...
test("phone Excel import is atomic, logged, service-role only, and prevents orphan field endpoints", () => {
  for (const marker of ["phone_terminal_import_logs", "import_phone_terminal_rows_v1", "phone_terminal_points_location_uidx", "source_rows", "failure_reasons", "現場端資料不得建立為沒有系統端的孤立資料"]) assert.ok(phoneImport.includes(marker), `missing phone import marker: ${marker}`);
